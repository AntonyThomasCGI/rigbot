...
class Colours:
	...
	def get_linear_value( colour ):
		## Converts colour space to linear
		# for item in Colours._colourRGB:
		# 	if item == colour:
		rgb_colour = copy.copy(Colours.get_rgb_value( colour ))
		# rgb_colour = copy.copy( Colours._colourRGB[item] )
...
def makeJointChain( length, name, suffix='jnt', rad=1 ):
	## length (int) = length of joint chain
	## name (string) = prefix naming convention
	## suffix (string) = suffix naming convention
	## rad (float, int) = radius of joints
...
def setOverrideColour( colour, *args ):
	## colour (string or 0-1 RGB) = colour of override
	## args (list) = nodes to change

	item_ls = makePynodeList( args, type=['transform', 'joint', 'nurbsCurve', 
											'locator'] )
...
def setOutlinerColour( colour, *args ):
	## colour (string or 0-1 RGB) = outliner colour
	## args (list) = nodes to change
...
def makePynodeList( *args, **kwargs ):
	## args (list of string and or pynode) = nodes
	## kwargs (string, list) = node type filter
...
def safeMakeChildGroups( path_ls, query=False ):
	## path_ls (list) = list of dag paths eg; ['Rig|joints', 'Rig|ctrls|etc.']
	## query (bool) = can run in query mode to return if groups exist
	## non-query returns pynode of specified path or creates node if doesn't exist
...
def makeAttrFromDict( ob, attr_data ):
	'''
	ob (pyNode) 		= node to add attribute to
	attr_data (dict)	= flags for addAttr/setAttr command
							
	- sets keyable=True and channelBox=True by default
	'''

	try:
		attr_name = attr_data.pop('name')
	except:
		raise UtilsException( '"name" was not specified in dictionary but is '
												+'required to make attribute.' )
